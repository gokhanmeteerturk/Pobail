...
from main.models import Member, Space
...
class Quiz(models.Model):
    ...
    def attempted_members(self):
        return Member.objects.filter(memberanswer__quiz=self).distinct().count()
...
class Question(models.Model):
    ...
    def total_correct_answers(self):
        return MemberAnswer.objects.filter(question=self, answer=self.answer).count()
...
class MemberAnswer(models.Model):
    ...
    def __str__(self):
        return self.member.name + ' ' + self.quiz.title + ' ' + self.question.question

    class Meta:
        unique_together = ('member', 'quiz', 'question')
